...
def create_random_collection(letters,random_dict_list): # define the method for creating the list of 10 random dictionaries
...
def create_common_dictionary(key_list, duplicates,random_dict_list):
    ...
    if len(duplicates) <= 0:  # the condition for checking if duplicates list is empty
        ...
        while y < len(random_dict_list):  # set the conditions of the loop
            ...
            for z in uniq:  # set the conditions of the loop
                if z in tested_dictionary:  # the condition for checking if keys is in the [y] dictionary
                    one_common_list[z] = tested_dictionary[
                        z]  # add item (key and value) to the one_common_list dictionary
        ...
        for m in duplicates:  # set the condition of the loop
            ...
            while k < len(random_dict_list):  # set the condition of the loop
                if m in random_dict_list[
                    k]:  # the condition for checking if key is in the k-dictionary in the random_dict_list
                    tested_dictionary = random_dict_list[
                        k]  # set tested_dictionary = [y] dictionary from the random_dict_list
                    final_key_name = str(m) + "_" + str(k + 1)  # set the final_key_name as concat m and (k+1) variables
                    duplicate_list[final_key_name] = tested_dictionary[
                        m]  # add item (key and value) to the one_common_list dictionary
                ...
                if k == len(random_dict_list):  # check if the k = the length of list
                    biggest_key = max(duplicate_list,
                                      key=duplicate_list.get)  # get the key with the biggest value from the duplicate_list dictionary
                    # print(biggest_key)
                    one_common_list[biggest_key] = duplicate_list[
                        biggest_key]  # add item (key and value) to the one_common_list dictionary
    ...
    if len(uniq) == len(one_common_list):  # check if the count of elements in the received list = count in the uniq
        ...
        return one_common_list
    else:  # if condition is false
        # print('Houston, we have a problem!')  # print the message about the error in console
        return 'Houston, we have a problem!'


create_random_collection(letters,random_dict_list)  # call the method create_random_collection
...
#print(create_random_collection(letters,random_dict_list))
